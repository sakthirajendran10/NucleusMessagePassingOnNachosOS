...
RunUserProg(void *filename) {
    ...
    if (space->Load((char*)filename)) {  // load the program into the space
        // cout<<"============================================="<<endl;
        // cout<<"Address space created and loaded successfully"<<endl;
        // cout<<"============================================="<<endl;
        cout<<"\n\nStarting execution of "<<(char *)filename<<endl;
        cout<<"=============================================\n"<<endl;
...
main(int argc, char **argv)
{
    int i;
    char *debugArg = "";
    vector<char *> userProgName;        // default is not to execute a user prog
    //char *prevflg="";
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
	    userProgName.push_back(argv[i + 1]);
        //prevflg="-x";
        //cout<<"userprog name: "<<userProgName<<endl;
	    i++;
	}
    ...
    // else{
    //     if(strcmp(prevflg, "-x") == 0){
    //         if(argv[i]!=NULL||!(strcmp(argv[i], "") == 0)){
    //             userProgName.push_back(argv[i]);
    //             //i++;
    //         }
    //     }
    // }
    ...
    if (userProgName.size()>0) {
      //RunUserProg(userProgName);
        for(size_t i=0;i<userProgName.size();++i){
            string str(userProgName[i]);
            //string s = "../test/";
            //string::size_type j = str.find(s);
            //if (j != std::string::npos)
            //str.erase(j, s.length());
            //char *cstr = new char[str.length() + 1];
            //strcpy(cstr, str.c_str());
            // char* progname= (char* )str.c_str();
            // cout<<progname;
            Thread *thread=new Thread(userProgName[i]);
            kernel->processMap[str]=thread;
            thread->Fork((VoidFunctionPtr)RunUserProg,(void *) userProgName[i]);
